...
void USART_Init(USART_Handle_t *pUSARTHandle)
{
	...
	USART_PeriControlClock (pUSARTHandle->pUSARTx, ENABLE);
...
void USART_PeripheralControl (USART_RegDef_t *pUSARTx, uint2_t cmd)
{
	if(cmd == ENABLE)
	{
		pUSARTx->CR1 |= (1<<13);
...
void USART_PeriControlClock(USART_RegDef_t *pUSARTx,uint8_t EnOrDi)
{
	if (EnOrDi == ENABLE)
	{
		if(pUSARTx == USART1)
...
void USART_ReceiveData(USART_Handle_t *pUSARTHandle, uint8_t *pRxBuffer, uint32_t Len)
{
	...
	for(uint32_t i = 0 ; i < Len; i++)
	{
		...
		//Check the USART_WordLength item for 9BIT or 8BIT in a frame
		if (pUSARTHandle->USART_Config.USART_WordLength == USART_USART_WORDLEN_9BITS)
		{
			...
			if(pUSARTHandle->USART_Config.USART_ParityControl == USART_PARITY_DISABLE)
			{
				...
				*((uint16_t*)pTxBuffer) = ((pUSARTHandle->pUSARTx->DR) & (uint16_t) 0x01FF);
